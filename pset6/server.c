...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* tmp_1 = strchr(line, ' ');
            
            if (tmp_1 != NULL)
            {
                if(strncmp("GET", line, tmp_1 - line) != 0)
                {
                    error(405);
                    continue;
                }
            ...
            char* tmp_2 = strchr(tmp_1 + 1, ' ');
            
            char target[tmp_2 - tmp_1];
            char* q;
            char abs_path[tmp_2 - tmp_1];
            
            if (tmp_2 != NULL)
            {
                strncpy(target, tmp_1+1, tmp_2 - tmp_1 - 1); 
                target[tmp_2 - tmp_1 - 1] = '\0';
                
                q = strchr(target, '?');
                
                int path_len = (q == NULL) ? strlen(target) : q - target;
                strncpy(abs_path, target, path_len);
                abs_path[path_len] = '\0';
                ...
                else if (strncmp("/", target, 1) != 0 || strchr(abs_path, '.') == NULL)
                {
                    error(501);
                    continue;
                }
            ...
            if (strncmp("HTTP/1.1", tmp_2+1, 8) != 0)
            {
                error(505);
                continue;
            }
